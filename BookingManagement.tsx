...
import type { Booking, BookingStatus, Driver, Session } from '../types';
...
const BookingTableRow: React.FC<{
    ...
    isUpdating: boolean;
}> = ({ booking, drivers, onViewDetails, onEdit, onAction, onQuickUpdate, isSelected, onSelect, isUpdating }) => {
    const { t } = useTheme();
    ...
    return (
        <tr onClick={handleRowClick} className={`transition-colors duration-200 cursor-pointer ${isSelected ? 'bg-primary-50 dark:bg-primary-500/10' : 'even:bg-white odd:bg-slate-50 dark:even:bg-slate-900 dark:odd:bg-slate-800/50'} hover:bg-slate-100 dark:hover:bg-slate-800 ${isUpdating ? 'opacity-60 pointer-events-none animate-pulse' : ''}`}>
            ...
            <td className="p-4 w-48">
                <select value={booking.driver || 'Unassigned'} onChange={(e) => onQuickUpdate(booking, { driver: e.target.value })} className="font-semibold appearance-none bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 w-full pl-3 pr-8 py-1.5 rounded-md focus:outline-none focus:ring-2 focus:ring-primary-500 text-sm border border-slate-300 dark:border-slate-700">
...
// FIX: Define BookingDetailModal component to resolve 'Cannot find name' error.
const BookingDetailModal: React.FC<{
    ...
    useEffect(() => {
        if (isOpen) {
            if (booking) {
                // Ensure form_data and datetime exist for editing
    ...
    const inputClasses = "mt-1 block w-full border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700/50 text-slate-900 dark:text-white focus:ring-primary-500 focus:border-primary-500";
    const labelClasses = "block text-sm font-medium text-slate-700 dark:text-slate-300";

    return (
        <Modal isOpen={isOpen} onClose={onClose} title={booking ? `Edit Booking #${booking.id}` : 'Create New Booking'}>
            <form onSubmit={handleSave} className="space-y-4">
                ...
                <div>
                    <label htmlFor="driver" className={labelClasses}>Assign Driver</label>
                ...
                <div className="flex justify-end space-x-2 pt-4">
                    <button type="button" onClick={onClose} className="px-4 py-2 bg-slate-200 text-slate-800 rounded-lg hover:bg-slate-300 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">{t('cancel', 'Cancel')}</button>
                    <button type="submit" disabled={isSaving} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 flex items-center justify-center disabled:opacity-70 w-36">
                        {isSaving 
                            ...
                            : (booking ? t('save_changes', 'Save Changes') : t('create_booking', 'Create Booking'))}
                    </button>
                </div>
            </form>
        </Modal>
            ...
            <div className="flex items-center justify-between">
             ...
             <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700/70">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <input type="text" placeholder="Search by ID, name, route..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full lg:col-span-2 bg-slate-50 dark:bg-slate-800 border-slate-200/70 dark:border-slate-700/70 rounded-lg placeholder:text-slate-400 focus:ring-primary-500 focus:border-primary-500"/>
                    <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as any)} className="w-full bg-slate-50 dark:bg-slate-800 border-slate-200/70 dark:border-slate-700/70 rounded-lg focus:ring-primary-500 focus:border-primary-500">
                        <option value="All">All Statuses</option>
                        {(['Scheduled', 'On Way', 'In Progress', 'Completed', 'Cancelled'] as BookingStatus[]).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                    <select value={sortBy} onChange={e => setSortBy(e.target.value as any)} className="w-full bg-slate-50 dark:bg-slate-800 border-slate-200/70 dark:border-slate-700/70 rounded-lg focus:ring-primary-500 focus:border-primary-500">
